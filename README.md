...
This Eurorack module SONUM is a fixed filter bank with a low-pass, 12 band-pass, and a high-pass filter with fixed cutoff frequences and resonances.
The core part of the module is based on the [fixed filter bank from Yusynth](http://yusynth.net/Modular/EN/BANK/index.html), inspired by the Moog 914 fixed filter bank.
...
Rich explained the intention of these different frequencies in a [YouTube video of his channel "Analog Output"](https://www.youtube.com/watch?v=-yv34qV_Hrs).
...
The SONUM module provides CV inputs for the volume of the 12 band-pass filter, as well as a feedback loop of the output signal back into the module input.
...
That enabled me to keep the module size at 28HP.
...
Usually, I prefer labeling components with their values on the PCBs. S I do not have to switch between BOM, schematic, and PCB while selecting parts in the soldering process.

However, due to the huge number of components for this module, the space on the main PCB is not sufficient to print all capacitor and resistor values.
So I decided to label them with the component numbers given in the schematic.
Those numbers are also listed in the BOM.
